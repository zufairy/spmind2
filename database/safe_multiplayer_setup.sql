-- Safe Multiplayer Setup for Supabase
-- This handles existing policies gracefully

-- Enable Realtime for the tables (required for real-time updates)
...
DROP POLICY IF EXISTS "Anyone can view public rooms" ON multiplayer_rooms;
DROP POLICY IF EXISTS "Anyone can create rooms" ON multiplayer_rooms;
DROP POLICY IF EXISTS "Room creators can update their rooms" ON multiplayer_rooms;
DROP POLICY IF EXISTS "Room creators can delete their rooms" ON multiplayer_rooms;
...
DROP TRIGGER IF EXISTS update_multiplayer_rooms_updated_at ON multiplayer_rooms;
